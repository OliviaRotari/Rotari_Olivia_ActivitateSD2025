...
void filtreazaHeap(Heap heap, int pozitieNod) {
	...
	if (pozitieNod >= 0 && pozitieNod < heap.nrMasini) {
		...
		if (pozStanga < heap.masini && heap.masini[pozMaxim].id < heap.masini[pozStanga].id) {
			pozMaxim = pozStanga;
		}
		if (pozDreapta < heap.masini && heap.masini[pozMaxim].id < heap.masini[pozDreapta].id) {
			pozMaxim = pozDreapta;
		}
...
Masina extrageMasina(Heap* heap) {//am modificat din void in pointer de Heap
